...
<img src="../res/01/tiobe_index.png" style="zoom:40%;">

<img src="../res/01/github_pypl_and_ieee_spectrum.png" style="zoom:60%;">
...
<img src="../res/01/python_download_page_1.png" style="zoom:40%;">
...
<img src="../res/01/python_download_page_2.png" style="zoom:36%;">

### Windows环境

下面我们以 Windows 11为例，讲解如何在 Windows 操作系统上安装 Python 环境。双击运行从官网下载的安装程序，会打开一个安装向导，如下图所示。

<img src="../res/01/install_python_1.png" style="zoom:50%;">
...
接下来，安装向导会提示你勾选需要的“Optional Featu../res/01”（可选特性），这里咱们可以直接全选。值得一提的是其中的第2项，它是 Python 的包管理工具 pip，可以帮助我们安装三方库和三方工具，所以一定要记得勾选它，然后点击“Next”进入下一环节。

<img src="../res/01/install_python_2.png" style="zoom:50%;">
...
<img src="../res/01/install_python_3.png" style="zoom:50%;">

安装成功会出现如下图所示的画面，安装成功的关键词是“successful”，如果安装失败，这里的单词会变成“failed”。

<img src="../res/01/install_python_4.png" style="zoom:50%;">
...
<img src="../res/01/install_python_5.png" style="zoom:50%;">
...
>
> <img src="../res/01/vs_build_tools_download.png" style="zoom:50%;">
>
...
>
> <img src="../res/01/vs_build_tools_install.png" style="zoom:50%;">

### 2.2 配置Visual Studio Code环境
...
### 2.4 安装和使用Anaconda
下载地址：https://www.anaconda.com/download/success  ，windows环境安装方法如下，
#### 一、登录官网下载Anacoda
https://www.anaconda.com/download/success
<img src="../res/01/anaconda_install_1.png" style="zoom:50%;">  

#### 二、版本要求
新的版本，对系统有一定的要求，支持的操操作系统：Windows 8或更高版本，64位macOS 10.13+或Linux，包括Ubuntu，RedHat，CentOS 6+等。  
其他系统下载低版本的anaconda软件，地址：https://repo.anaconda.com/archive/  
<img src="../res/01/anaconda_install_2.png" style="zoom:50%;">

#### 三、安装步骤
1.找到安装包（Anaconda3-2020.11-Windows-x86_64.exe）  
2.双击安装包，跳出如下窗口  
<img src="../res/01/anaconda_install_3.png" style="zoom:50%;">  
3.点击next，出现如下图，  
<img src="../res/01/anaconda_install_4.png" style="zoom:50%;">  
4.点击I Agree，出现下图，  
<img src="../res/01/anaconda_install_5.png" style="zoom:50%;">   
5.next,  
<img src="../res/01/anaconda_install_6.png" style="zoom:50%;">   
6.点击Browse..，修改自己的安装路径，我放在d盘，如图,  
<img src="../res/01/anaconda_install_7.png" style="zoom:50%;">   
7.点击next，出现如图，**两个复选框均勾选上（一定要勾选）**，如图  
<img src="../res/01/anaconda_install_8.png" style="zoom:50%;">   
8.点击Install，进行安装，如图  
<img src="../res/01/anaconda_install_9.png" style="zoom:50%;">  
9.结束后，点击next，出现如图  
<img src="../res/01/anaconda_install_10.png" style="zoom:50%;">  
10.next,  
<img src="../res/01/anaconda_install_11.png" style="zoom:50%;">  
11.点击finish，安装完成。
...
#### 一、如何打开Jupyter
方法一：  
<img src="../res/01/jupyter_1.png" style="zoom:50%;">   
<img src="../res/01/jupyter_2.png" style="zoom:50%;">   
<img src="../res/01/jupyter_3.png" style="zoom:50%;">  
方法二：  
1.找到我们要打开的文件目录，我要找到python_notes，那么我就要在磁盘中进入这个目录  
<img src="../res/01/jupyter_4.png" style="zoom:50%;">  
2.将鼠标定位在路径上，如图  
<img src="../res/01/jupyter_5.png" style="zoom:50%;">  
3.在路径栏输入cmd，如图   
<img src="../res/01/jupyter_6.png" style="zoom:50%;">  
4.然后按键盘上的回车键，出现命令行的黑色窗口，如图  
<img src="../res/01/jupyter_7.png" style="zoom:50%;">  
5.在命令窗口输入：jupyter notebook  
<img src="../res/01/jupyter_8.png" style="zoom:50%;">  
6.然后按回车键，会在浏览器打开，这就到了和我们平时用的一样的界面了  
<img src="../res/01/jupyter_9.png" style="zoom:50%;">  

#### 二、Jupyter的具体介绍  
参考：[【Jupyter Notebook】详尽介绍](https://blog.csdn.net/qubick/article/details/140771074?spm=1001.2014.3001.5501)


### 2.6 说明

有人可能会推荐新手直接安装 Anaconda，因为 Anaconda 会帮助我们安装 Python 解释器以及一些常用的三方库，除此之外还提供了一些便捷的工具，特别适合萌新小白。我个人并不推荐这种方式，因为在安装 Anaconda 时你会莫名其妙安装了一大堆有用没用的三方库（占用比较多的硬盘空间），然后你的终端或命令提示符会被 Anaconda 篡改（每次启动自动激活虚拟环境），这些并不符合软件设计的**最小惊讶原则**。其他关于 Anaconda 的小毛病此处就不再赘述了，如果你非要使用 Anaconda，推荐安装 Miniconda，它跟 Anaconda 在同一个下载页面。
...
### 2.7 配置说明（如何在Windows系统中配置深度学习环境）
1.硬件要求  
通常我们对硬件没有太多要求，尤其是学习期间只要硬件和软件运行稳定即可。如果有条件可以参考以下配置（仅作参考，不是硬性要求）：  
- CPU——AMDRyzen74800HS8C-16T@4.2GHzonTurbo  
- RAM——16GBDDR4RAM@3200MHz  
- GPU——NvidiaGeForceRTX2060Max-Q@6GBGDDR6显存  
  
2.软件要求  
操作系统最好是WIN11版本，如果还在使用WIN10也可以。WIN10之前例如WIN7,WIN8的系统建议进行升级。这类太早期的系统对后续学习期间使用的软件会有冲突和兼容问题。不建议继续用来学习AI相关课程。

## 三、基础语法

### 1.编码
python3源码文件默认是以UTF-8编码，所有字符串都是unicode字符串。也支持指定不同的源码，  
``` 
#-*- coding: cp-1252 -*-
``` 
### 2.标识符
* 第一个字符必须以字母（a-z,A-Z）或下划线组成_;
* 标识符的其他部分由字母、数字、下划线组成；
* 标识符对大小写敏感；
* 标识符长度无限制；
* 禁止使用保留关键字作为标识符，如：if、for等 
...
### 5.行与缩进
1.python最具特色的就是使用缩进来表示代码块，不需要使用大括号 {} 。缩进的空格数是可变的，但是同一个代码块的语句必须包含相同的缩进空格数。  
2.多行语句：通常是一行写完一条语句，但如果语句很长，我们可以使用反斜杠 \ 来实现多行语句，例如：  
```
total = item_one + \
        item_two + \
        item_three
...
```
3.同一行显示多条语句：语句之间使用分号 ; 分割，以下是一个简单的实例：  
```
#!/usr/bin/python3
